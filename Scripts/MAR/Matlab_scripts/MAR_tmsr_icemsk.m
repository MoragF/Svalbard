function MAR_tmsr_icemsk(percentage)
...
%NOTE: this code uses ice mask that has been interpolated on the rgi grid
    ...
    for i=10:20
        ...
        for j=1:length(t)
            ...
            sm = SM(:,:,j);
            me = ME(:,:,j);
            SMB_tmsr(x)= mean(smb(comb_mask==1),'omitnan');
            RF_tmsr(x)= mean(rf(comb_mask==1),'omitnan');
            SF_tmsr(x)= mean(sf(comb_mask==1),'omitnan');
            TSH_tmsr(x)= mean(tsh(comb_mask==1),'omitnan');
            RU_tmsr(x)= mean(ru(comb_mask==1),'omitnan');
            SU_tmsr(x)= mean(su(comb_mask==1),'omitnan');
            SC_tmsr(x)= mean(sc(comb_mask==1),'omitnan');
            SM_tmsr(x)= mean(sm(comb_mask==1),'omitnan');
            ME_tmsr(x)= mean(me(comb_mask==1),'omitnan');
        end

    end
        file = '/exports/csce/datastore/geos/groups/geos_EO/Databases/MAR/Svalbard-RA/Svalbard_IceArea/Svalbard_25.nc';
        nt = length(time);
