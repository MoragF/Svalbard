function MAR_tmsr_icemsk_r
%MAR_tmsr_icemsk_r--> average timeseries fro each region using interpolated
%icemask as well as region masks (default <25% ice coverage)
%variables
%NOTE: this code uses original MAR data that has not been interpolated ice mask 
    for r=1:6
        [a,m] = readfile(['/exports/csce/datastore/geos/groups/geos_EO/L0data/GLIMS/RGI_6.0/Svalbard_Masks/uncorr/07_rgi60_Svalbard_land_R',num2str(r),'.tif']);
        time=[];
        x=0;
        ...
        for i=10:20
            ...
            for j=1:length(t)
                ...
                sm = SM(:,:,j);
                me = ME(:,:,j);
                SMB_tmsr(x)= mean(smb(comb_mask==1));
                RF_tmsr(x)= mean(rf(comb_mask==1));
                SF_tmsr(x)= mean(sf(comb_mask==1));
                TSH_tmsr(x)= mean(tsh(comb_mask==1));
                RU_tmsr(x)= mean(ru(comb_mask==1),'omitnan');
                SU_tmsr(x)= mean(su(comb_mask==1));
                SC_tmsr(x)= mean(sc(comb_mask==1));
                SM_tmsr(x)= mean(sm(comb_mask==1));
                ME_tmsr(x)= mean(me(comb_mask==1));
            end

        end
        file = ['/exports/csce/datastore/geos/groups/geos_EO/Databases/MAR/Svalbard-RA/Svalbard_Masked/Nearest_Tmsr/Svalbard_LR',num2str(r),'.nc'];
        nt = length(time);
